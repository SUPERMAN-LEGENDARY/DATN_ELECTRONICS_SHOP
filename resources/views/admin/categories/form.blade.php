...
@push('styles')
<style>
.form-card { background:#fff; border-radius:10px; box-shadow:0 1px 4px rgba(0,0,0,.08);
             max-width:600px; padding:28px; }
...
.form-control { width:100%; padding:9px 12px; border:1px solid #ddd; border-radius:6px;
...
.invalid-feedback { color:#C62828; font-size:12px; margin-top:4px; }

/* Logo preview */
.logo-preview-wrap { margin-top:10px; }
.logo-preview { width:80px; height:80px; object-fit:contain; border:1px solid #eee;
                border-radius:8px; padding:4px; display:block; }
.logo-hint { font-size:12px; color:#999; margin-top:6px; }

/* Toggle switch */
.toggle-wrap { display:flex; align-items:center; gap:10px; }
.toggle-input { position:relative; width:44px; height:24px; }
.toggle-input input { opacity:0; width:0; height:0; }
.toggle-slider { position:absolute; inset:0; background:#ccc; border-radius:12px;
                 cursor:pointer; transition:.2s; }
.toggle-slider:before { content:''; position:absolute; width:18px; height:18px;
                         left:3px; top:3px; background:#fff; border-radius:50%; transition:.2s; }
.toggle-input input:checked + .toggle-slider { background:#1565C0; }
...
.btn { display:inline-flex; align-items:center; gap:6px; padding:9px 20px; border-radius:6px;
       font-size:14px; font-weight:600; cursor:pointer; border:none; text-decoration:none; transition:.15s; }
