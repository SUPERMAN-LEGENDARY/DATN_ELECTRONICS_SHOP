...
<style>
    ...
    .btn-attr-del:disabled {
        color: #ccc;
        cursor: not-allowed;
    }

    .btn-attr-del:disabled:hover {
        background: none;
    }
...
<script>
    ...
    function addAttr() {
        ...
        if (!name) {
            showModalError('Vui lòng nhập tên thuộc tính.');
            return;
        }
        ...
        fetch(ROUTES.store, {
            ...
            .then(data => {
                if (data.errors) {
                    showModalError(Object.values(data.errors)[0][0]);
                    return;
                }
                input.value = '';
    ...
    (function() {
        ...
        // Xóa lỗi khi người dùng nhập/chọn
        ['name', 'category_id', 'brand_id', 'price'].forEach(id => {
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (!category || !category.value) {
                setError('category_id', 'Vui lòng chọn danh mục.');
                isValid = false;
            } else {
            ...
            if (!brand || !brand.value) {
                setError('brand_id', 'Vui lòng chọn thương hiệu.');
                isValid = false;
            } else {
                clearError('brand_id');
            }
            ...
            const priceVal = parseFloat(price ? price.value : '');
            if (!price || price.value === '' || isNaN(priceVal) || priceVal < 0) {
                setError('price', 'Vui lòng nhập giá gốc hợp lệ (≥ 0).');
                isValid = false;
            } else {
                clearError('price');
            }
            ...
            if (!isValid) {
                ...
                const firstError = form.querySelector('[style*="border-color: rgb(229, 57, 53)"], [style*="border-color:#E53935"]');
